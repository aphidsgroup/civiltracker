...
const routes = [
  '/sites/new', '/sites/[id]',
  '/bills/upload', '/bills/[id]',
  '/expenses/new',
  '/labour/attendance', '/labour/salary',
  '/materials/requests',
  '/purchase', '/vendors', '/subcontractors', '/boq', '/tasks', '/documents', '/clients', '/approvals'
];

const basePath = path.join(__dirname, 'src', 'app', '(dashboard)');

routes.forEach(route => {
  const fullPath = path.join(basePath, ...route.split('/'));
  fs.mkdirSync(fullPath, { recursive: true });
  
  const title = route.split('/').filter(Boolean).pop().replace(/-/g, ' ').replace(/\[id\]/, 'Details').toUpperCase();
