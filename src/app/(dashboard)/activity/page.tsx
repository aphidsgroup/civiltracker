  ...
  if (siteIds.length > 0) {
    const [expenses, dprs, attendance, contractorAttendances, photos] = await Promise.all([
      prisma.expense.findMany({ 
        where: { siteId: { in: siteIds }, createdAt: { gte: startOfToday } }, 
      ...
      prisma.labourAttendance.findMany({ 
        where: { labour: { siteId: { in: siteIds } }, date: { gte: startOfToday } }, 
        include: { labour: { include: { site: true } } } 
      }),
      prisma.contractorAttendance.findMany({ 
        where: { siteId: { in: siteIds }, date: { gte: startOfToday } }, 
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Real-time feed of all updates happening across your sites today.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 min-h-[400px]">
        {activities.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-20 opacity-60">
            ...
            <h3 className="text-base font-bold text-slate-700 m-0">No activities recorded today</h3>
            <p className="text-sm font-medium text-slate-500 mt-1 max-w-[250px]">
              Updates made across all your sites will appear here in chronological order.
            </p>
          </div>
        ) : (
