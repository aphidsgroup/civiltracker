  ...
  return (
    <div className="flex flex-col gap-5.5 mt-4">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-extrabold m-0 mb-1 tracking-tight text-slate-900">Site Expenses</h2>
          <p className="text-slate-500 text-xs m-0">{expenses.length} records · {pending} pending approval</p>
        </div>
        <Link href={`/mobile/add/expense?siteId=${siteId}`} className="inline-flex items-center gap-1.5 bg-[#fc6e20] text-white rounded-lg px-4 py-2 text-sm font-bold hover:bg-[#e85b0d] transition-colors shadow-sm no-underline">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {expenses.length === 0 ? (
            <div className="text-center py-12 bg-slate-50">
              ...
              <div className="text-sm text-slate-500 mt-1 mb-4">There are no expenses tracked for this site yet.</div>
              <Link href={`/mobile/add/expense?siteId=${siteId}`} className="inline-flex font-bold text-sm text-[#fc6e20] hover:text-[#e85b0d]">
                + Add an expense
              </Link>
            </div>
          ) : (
        ...
        {expenses.length === take && (
          <div className="p-4 flex justify-center border-t border-slate-200 bg-slate-50">
            <Link href={`/sites/${siteId}/expenses?limit=${take + 50}`} className="text-sm font-bold text-[#fc6e20] hover:underline">
              Load More Records
            </Link>
          </div>
        )}
